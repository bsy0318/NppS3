...
const wchar_t* const kEN[N] = {
    ...
    L"Language changes fully apply after restarting Notepad++.",
...
const wchar_t* const kKO[N] = {
    ...
    L"프로필 \"%s\"을(를) 삭제할까요?\n저장된 비밀 키도 함께 제거됩니다.",
    L"언어 변경은 Notepad++ 재시작 후 완전히 적용됩니다.",
...
const wchar_t* const kJA[N] = {
    ...
    L"プロファイル「%s」を削除しますか?\n保存された秘密キーも削除されます。",
    L"言語の変更は Notepad++ の再起動後に完全に適用されます。",
...
const wchar_t* const kRU[N] = {
    ...
    L"Смена языка полностью применяется после перезапуска Notepad++.",
